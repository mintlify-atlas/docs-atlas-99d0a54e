---
title: "Get Started with Avanzar In Time Shop"
sidebarTitle: "Quickstart"
description: "Set up the full Avanzar In Time Shop stack locally — database, backend, admin dashboard, and storefront — in under 10 minutes."
---
...
<Steps>
  ...
  <Step title="Create an admin user">
    Seed the initial admin account using the bootstrap script:

    ```bash
    bun run seed:admin
    ```

    This executes `apps/backend/src/scripts/create-admin.ts` and creates a user with the `admin` role. Follow the prompts in your terminal to set the email and password.
  </Step>
