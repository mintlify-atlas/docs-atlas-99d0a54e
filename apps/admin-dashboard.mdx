---
title: "Admin Dashboard: React SPA for Store Management"
sidebarTitle: "Admin Dashboard"
description: "The Avanzar admin dashboard is a React 19 SPA with TanStack Query, Radix UI, and Tailwind v4 for managing products, orders, payments, and users."
---
...
<Note>
  The admin app proxies all `/api/*` requests to the backend running at `http://localhost:3000`. The backend must be running before you open the dashboard. To change the backend URL, update the `apiFetch` base in `apps/admin/src/lib/api.ts`.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Categories" icon="tag">
    Create and manage product categories. The category model supports a parent–child hierarchy for building nested navigation trees in the storefront.
  </Card>
  <Card title="Shipping Rates" icon="truck">
    Define per-province shipping rates in multiple currencies. Rates are consumed by the public API and used during checkout to compute delivery costs.
  </Card>
  <Card title="Users" icon="users">
    Browse registered users and update their roles (e.g. promote a user to `admin`). A guard on the backend prevents admins from downgrading their own role.
  </Card>
</CardGroup>
...
All HTTP calls go through the `apiFetch` wrapper in `apps/admin/src/lib/api.ts`. It uses the browser's native `fetch` with `credentials: "include"` so the session cookie is attached to every request, and parses non-2xx responses into typed `ApiError` instances:
